...
class PairwiseFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, in_channels):
        ...
        self.obj_co_occurrence = torch.load("/home/user/hetsgg-torch-master/obj_co_occurrence_matrix.pt").cuda()
        self.obj_co_occurrence = self.obj_co_occurrence[1:, 1:] / torch.diag(self.obj_co_occurrence)[1:]
        print(self.obj_co_occurrence)
    ...
    def pairwise_rel_features(self, augment_obj_feat, union_features, rel_pair_idxs, inst_proposals):
        ...
        for pair_idx, head_rep, tail_rep, obj_box in zip(rel_pair_idxs, head_reps, tail_reps, obj_boxs):
            # CooK
            # weight = self.obj_co_occurrence[pair_idx[:, 0], pair_idx[:, 1]]
            # obj_pair_feat4rel_rep.append(weight.unsqueeze(-1) * torch.cat((head_rep[pair_idx[:, 0]], tail_rep[pair_idx[:, 1]]), dim=-1))
...
#relation-centric
class DualGraphFeatureExtractor(nn.Module):
    """
    extract the pairwise features from the object pairs and union features.
    most pipeline keep same with the motifs instead the lstm massage passing process
    """
    ...
    def __init__(self, config, in_channels):
        super(DualGraphFeatureExtractor, self).__init__()
        self.cfg = config
        statistics = get_dataset_statistics(config)
        obj_classes, rel_classes = statistics['obj_classes'], statistics['rel_classes']
        self.num_obj_classes = len(obj_classes)
        ...
        # self.rel_co_occurrence = torch.load("/home/clkim/edgesgg/rel_co_occurrence_matrix.pt").cuda()
        # # print(torch.diag(co_occurrence_matrix)[1:])
        # self.rel_co_occurrence = self.rel_co_occurrence[1:, 1:] / torch.diag(self.rel_co_occurrence)[1:]
        # print(self.rel_co_occurrence)
        ...
        if self.rel_feature_type in ["obj_pair", "fusion"]:
            ...
            self.pairwise_edge_feat_finalize_fc = nn.Sequential(
                make_fc(self.hidden_dim * 6, self.pooling_dim),
                nn.ReLU(inplace=True),
            )
    ...
    def pairwise_rel_features(self, augment_obj_feat, union_features, rel_pair_idxs, inst_proposals):
        ...
        for pair_idx, head_rep, tail_rep, obj_box in zip(rel_pair_idxs, head_reps, tail_reps, obj_boxs):
            ...
            # if len(pair_idx[pair_idx[:,0] > 50]) > 1:
            #     print(pair_idx[:, 0])
            #     print(pair_idx[:, 1])

            # weight = self.rel_co_occurrence[pair_idx[:, 0], pair_idx[:, 1]]
            obj_pair_feat4rel_rep.append(torch.cat((head_rep[pair_idx[:, 0]], tail_rep[pair_idx[:, 1]]), dim=-1))
            pair_bboxs_info.append(get_box_pair_info(obj_box[pair_idx[:, 0]], obj_box[pair_idx[:, 1]]))
    ...
    def edge_pairwise_rel_features(self, augment_obj_feat, rel_pair_idxs, rel_features, e2e_pair_idx, inst_proposals):
        obj_boxs = [get_box_info(p.bbox, need_norm=True, proposal=p) for p in inst_proposals]
        num_objs = [len(p) for p in inst_proposals]
        num_edges = [len(e) for e in rel_pair_idxs]
        # post decode
        # (num_objs, hidden_dim) -> (num_objs, hidden_dim * 2)
        # going to split single object representation to sub-object role of relationship
        pairwise_obj_feats_fused = self.pairwise_obj_feat_updim_fc(augment_obj_feat)
        ...
        split_edge_features = rel_features.split(num_edges, dim=0)

        obj_pair_feat4rel_rep = []

        for rel_pair_idx, e2e_pair, head_rep, tail_rep, edge_feature in zip(rel_pair_idxs, e2e_pair_idx, head_reps, tail_reps, split_edge_features):
            obj_pair_feat4rel_rep.append(torch.cat((head_rep[rel_pair_idx[:, 0]], tail_rep[rel_pair_idx[:, 1]], edge_feature), dim=-1))
        obj_pair_feat4rel_rep = cat(obj_pair_feat4rel_rep, dim=0)  # (num_rel, hidden_dim * 2)

        # if self.spatial_for_vision:
        #     obj_pair_feat4rel_rep = obj_pair_feat4rel_rep * self.spt_emb(pair_bbox_geo_info)

        obj_pair_feat4rel_rep = self.pairwise_edge_feat_finalize_fc(obj_pair_feat4rel_rep)  # (num_rel, hidden_dim)

        return obj_pair_feat4rel_rep

    def dualGraphConvertor(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):

        rel_pair_e2e = []
        dual_graph_pair = []
        for i, pair_idx in enumerate(rel_pair_idxs):
            for num_box in range(len(inst_proposals[i].bbox)):
                rel_pair_e2e.append(pair_idx[torch.where(pair_idx[:, 0] == num_box)[0]])
            dual_graph_pair.append(rel_pair_e2e)
            rel_pair_e2e = []

        return dual_graph_pair

    def dualGraphConvertor2(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):
    ...
    def forward(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):
        ...
        e2e_pair_idx = self.dualGraphConvertor(inst_roi_feats, union_features, inst_proposals, rel_pair_idxs)
        ...
        if self.rel_feature_type == "obj_pair" or self.rel_feature_type == "fusion":
            rel_features = self.pairwise_rel_features(augment_obj_feat, union_features,
                                                      rel_pair_idxs, inst_proposals)
            edge_rel_feature = self.edge_pairwise_rel_features(augment_obj_feat, rel_pair_idxs, rel_features,
                                                               e2e_pair_idx, inst_proposals)
            edge_rel_feature = rel_features + edge_rel_feature
            if self.rel_feature_type == "fusion":
                if self.rel_feat_dim_not_match:
                    union_features = self.rel_feature_up_dim(union_features)
                edge_rel_feature = union_features + rel_features

        elif self.rel_feature_type == "union":
            if self.rel_feat_dim_not_match:
                union_features = self.rel_feature_up_dim(union_features)
            edge_rel_feature = union_features
        ...
        return augment_obj_feat, edge_rel_feature
