...
from hetsgg.modeling.roi_heads.relation_head.model_msg_passing import (
    PairwiseFeatureExtractor, DualGraphFeatureExtractor
)
...
from hetsgg.layers.ltf_idf import TfIdfLayer
...
class HetSGGplus_Context(nn.Module):
    def __init__(
        ...
        self.vg_cat_dict = json.load(open(f'{cfg.DATA_DIR}/VG-SGG-Category_v2.json', 'r')) # VG-SGG-Category_v2.json or Category_Type_Info.json
        ...
        self.dualGraph_feature_extractor = DualGraphFeatureExtractor(cfg, in_channels)
        ...
        self.relation_cat = nn.Sequential(
            make_fc(self.pooling_dim*2, self.pooling_dim),
            nn.ReLU(True),
        )
        ...
        self.tf_idf = TfIdfLayer(bias=True)
        ...
        self.edge_downdim_fc = nn.Sequential(
            make_fc(self.pooling_dim, self.hidden_dim),
            nn.ReLU(True),
        )
        ...
        self.edge_pair2rel_fuse = nn.Sequential(
            nn.BatchNorm1d(self.hidden_dim * 2),
            make_fc(self.hidden_dim * 2, self.hidden_dim),
            nn.ReLU(),
        )
    ...
    def forward(
        ...
        # augment_obj_feat = self.tf_idf(augment_obj_feat, proposals)

        _, dual_edge_feature = self.dualGraph_feature_extractor(
            inst_features,
            rel_union_features,
            proposals,
            rel_pair_inds,
        ...
        refine_rel_feats_each_iters = [self.relation_cat(torch.cat((rel_feats, dual_edge_feature), dim=-1))]
...
class EdgeSGG_Context(nn.Module):
    def __init__(
            self,
            cfg,
            in_channels,
        ...
        super(EdgeSGG_Context, self).__init__()
        self.cfg = cfg
        self.hidden_dim = hidden_dim
        self.update_step = num_iter
        self.n_ntypes = cfg.MODEL.ROI_RELATION_HEAD.HETSGG.NUM_NODETYPES
        ...
        self.vg_cat_dict = json.load(open(f'{cfg.DATA_DIR}/VG-SGG-Category_v2.json', 'r'))
        ...
        self.gate_sub2pred = nn.Sequential(
            *[MessagePassingUnit(self.hidden_dim, gate_width, self.n_basis) for _ in range(param_set_num)])
        ...
        if self.gating_with_relness_logits:
            MessagePassingUnit = RelationMessagePassingUnitGatingWithRelnessLogits
            self.gate_pred2sub = nn.Sequential(
                *[MessagePassingUnit(self.hidden_dim, self.num_rel_cls, self.relness_weighting_mp, self.n_basis) for _
                  in range(param_set_num)])
            self.gate_pred2obj = nn.Sequential(
                *[MessagePassingUnit(self.hidden_dim, self.num_rel_cls, self.relness_weighting_mp, self.n_basis) for _
                  in range(param_set_num)])
    ...
    def _prepare_adjacency_matrix(self, proposals, rel_pair_idxs, relatedness):
        ...
        for idx, (prop, rel_ind_i) in enumerate(zip(proposals, rel_pair_idxs, )):
            ...
            if self.mode in ['sgcls', 'sgdet']:
                ...
                prop_node_types = self.vg_map_arr[prop.extra_fields['labels'].long(), 1].detach().cpu().numpy()
            # prop_node_types = torch.argmax(prop.get_field('category_scores'), -1).cpu().numpy()
            edge_type.append(torch.LongTensor([self.obj2rtype[(s, d)] for (s, d) in
                                               prop_node_types[rel_ind_i.detach().cpu().numpy() - offset]]).cuda())
            edge_type_inv.append(torch.LongTensor([self.obj2rtype[(d, s)] for (s, d) in
                                                   prop_node_types[rel_ind_i.detach().cpu().numpy() - offset]]).cuda())
            offset += len(prop)
            rel_inds_batch_cat.append(rel_ind_i)
