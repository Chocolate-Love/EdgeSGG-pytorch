...
class HetSGG(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        if self.score_update_step > 0:
            self.rmp_score = RMPLayer(num_classes_obj, num_classes_pred, num_classes_obj, num_classes_pred,
                                      num_bases=self.num_bases, num_relations=self.n_reltypes, cfg=cfg)
    ...
    def forward(self, inst_features, rel_union_features, proposals, rel_pair_inds,
                rel_gt_binarys=None, logger=None, is_training=True):
        ...
        for _ in range(self.score_update_step):
            obj_class_logits, pred_class_logits = self.rmp_score(g_rel, obj_class_logits, pred_class_logits,
                                                                 edge_type_list)
    ...
    def _get_map_idxs(self, proposals, rel_pair_inds, is_training):
        ...
        for proposal, rel_pair_ind in zip(proposals, rel_pair_inds):
            ...
            edge_type = torch.LongTensor(
                [self.obj2rtype[(s.item(), d.item())] for (s, d) in proposal_category[rel_ind_i.detach() - offset]])
            edge_type_inv = torch.LongTensor(
                [self.obj2rtype[(d.item(), s.item())] for (s, d) in proposal_category[rel_ind_i.detach() - offset]])
...
class RMPLayer(MessagePassing):
    ...
    def __init__(self, node_in_channels, edge_in_channels, node_out_channels, edge_out_channels, num_bases,
                 num_relations, cfg, bias=False):
    ...
    def propagate(self, edge_index, nf, ef, edge_type_list, size=None):
        ...
        obj2rel_msg = torch.sum(self.message(edge_index[0, :], None, obj2rel_x_j, x_ij, edge_type_rel_inv, 'obj2rel'),
                                -1)
        ...
        for rel in range(self.num_relations):
            ...
            node_mask_sub[:, rel] += self.aggregate((edge_type_rel_inv == rel).float()[..., None],
                                                    index=edge_index[0, :], ptr=None, dim_size=nf.size(0)).squeeze()
            node_mask_obj[:, rel] += self.aggregate((edge_type_rel == rel).float()[..., None], index=edge_index[1, :],
                                                    ptr=None, dim_size=nf.size(0)).squeeze()
