...
from hetsgg.modeling.roi_heads.relation_head.model_HetSGGplus import EdgeSGG_Context
...
@registry.ROI_RELATION_PREDICTOR.register("EdgeSGG_Predictor")
class EdgeSGG_Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(EdgeSGG_Predictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS
        ...
        if self.split_context_model4inst_rel:
            self.obj_context_layer = EdgeSGG_Context(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM,
            )
            self.rel_context_layer = EdgeSGG_Context(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM,
            ...
            self.context_layer = EdgeSGG_Context(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM,
