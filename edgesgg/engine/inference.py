...
from hetsgg.data.datasets.evaluation import evaluate
...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None, logger=None):
    ...
    for _, batch in enumerate(tqdm(data_loader, position=0, leave=True)):
        with torch.no_grad():
...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu, synchronize_gather=True):
    ...
    if len(image_ids) != image_ids[-1] + 1:
        ...
        logger = logging.getLogger("hetsgg.inference")
        logger.warning(
